...
function ProductCardVarient({ product, variant = "vertical" }) {
	...
	// Default vertical variant
        return (
                <Card onClick={handleViewProduct} className="w-full h-full hover:shadow-lg transition-shadow duration-300 cursor-pointer">
                        <CardContent className="h-full p-0 flex flex-col">
                                {/* Product Info Header */}
                                <div className="flex justify-between items-start p-4 md:p-6 flex-shrink-0">
                                        <div className="flex-1">
                                                <h3 className="font-bold text-base md:text-lg mb-1 line-clamp-2">{product?.title}</h3>
                                                {product?.subtitle && (
                                                ...
                                                <div className="flex flex-col mb-4">
                                                        {isAuthenticated ? (
                                                                <>
                                                                        <p className="flex font-bold text-xl md:text-2xl mb-2">₹{product?.price}</p>
                                                                        {product?.originalPrice && (
                                                                                <div className="flex items-center">
                                                                                        <p className="text-gray-500 line-through text-sm">₹{product?.originalPrice}</p>
                                                                                        <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded ml-2">25% OFF</span>
                                                                                </div>
                                                                        )}
                                                                </>
                                                        ) : (
                                                                <p className="text-base text-gray-700 italic">Please login to see price</p>
                                                        )}
                                                </div>
                                        </div>
                                        {product?.colors && (
                                                <div className="flex space-x-1 flex-shrink-0 ml-2">
                                                        {product?.colors.map((color, i) => (
                                                                <div key={i} className={`w-3 h-3 rounded-full ${color === "blue" ? "bg-blue-500" : color === "black" ? "bg-black" : color === "red" ? "bg-red-500" : "bg-orange-500"}`}/>
                                                        ))}
                                                </div>
                                        )}
                                </div>
                                ...
                                <div className="flex-1 px-4 md:px-6 mb-4">
                                        <div className="w-full h-full overflow-hidden">
                                                <img src={product?.image || "https://res.cloudinary.com/drjt9guif/image/upload/v1755168534/safetyonline_fks0th.png"} alt={product?.title} className="w-full h-full object-cover rounded-lg" />
                                        </div>
                                </div>
