...
const InvoiceDocument = ({ order }) => {
  ...
  taxAmountStr = formatCurrency(productIgst);
                                        ...
                                        taxType = `CGST ${cgstRate.toFixed(0)}%\nSGST ${sgstRate.toFixed(0)}%`;            // <-- single \n
                                        taxAmountStr = `${formatCurrency(productCgst)}\n${formatCurrency(productSgst)}`;  // <-- single \n
                                        }

                                       const productTax =
                                               productCgst + productSgst + productIgst;
                       ...
                       <View style={styles.totalSection}>
                               ...
                               {totalTaxLines.map((tax, idx) => (
                                       <View style={styles.totalRow} key={idx}>
                                               <Text>{`${tax.label}:`}</Text>
                                               <Text style={styles.alignRight}>{formatCurrency(tax.amount)}</Text>
                                       </View>
                               ))}
