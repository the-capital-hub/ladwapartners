...
const InvoiceDocument = ({ order }) => {
       ...
       const invoiceTaxType =
               order.gst?.igst > 0
                       ...
                       ? `CGST ${cgstRate.toFixed(0)}% & SGST ${sgstRate.toFixed(0)}%`
                       : "No Tax";
					...
					<Text>Order Status: {order.status.toUpperCase()}</Text>
                       ...
                       <View style={[styles.table, styles.fullWidth]}>
                               <View style={[styles.tableRow, styles.tableHeader]}>
                                       ...
                                       <View style={[styles.tableCol, styles.tableColSmall]}>
                                               <Text style={[styles.tableCell, styles.alignRight]}>Tax Amount</Text>
                                       </View>
                                       <View style={[styles.tableCol, styles.tableColSmall]}>
                                               <Text style={[styles.tableCell, styles.alignRight]}>Amount with Tax</Text>
                                       </View>
                               </View>
                               ...
                               {order.products.map((product, index) => {
                                       const taxType =
                                               order.gst?.igst > 0
                                                       ...
                                                       : "";
                                       const productTax =
                                               invoiceTaxAmount > 0 && order.subtotal > 0
                                                       ? (product.totalPrice / order.subtotal) * invoiceTaxAmount
                                                       : 0;
                       ...
                       <View style={styles.totalSection}>
                               <View style={styles.totalRow}>
                                       <Text>Subtotal: {formatCurrency(order.subtotal)}</Text>
                               </View>
                               <View style={styles.totalRow}>
                                       <Text>Tax Type: {invoiceTaxType}</Text>
                               </View>
                               {invoiceTaxAmount > 0 && (
                                       <View style={styles.totalRow}>
                                               <Text>Tax Amount: {formatCurrency(invoiceTaxAmount)}</Text>
                                       </View>
                               )}
                               {order.shippingCost > 0 && (
                                       <View style={styles.totalRow}>
                                               <Text>Shipping: {formatCurrency(order.shippingCost)}</Text>
                                       </View>
                               )}
                               {order.discount > 0 && (
                                       <View style={styles.totalRow}>
                                               <Text>Discount: -{formatCurrency(order.discount)}</Text>
                                       </View>
                               )}
